...
float randn_internal_controllerX(long *idum, int reset) {
  ...
  if (iset==0) {
    do {
      v1 = 2.0*ran1d_controllerX(idum, reset)-1.0;
      v2 = 2.0*ran1d_controllerX(idum, reset)-1.0;
      rsq = v1*v1+v2*v2;
...
double randn_controllerX(void) {
  return randn_internal_controllerX(&global_seed, 0);
}
void reset_rand_controllerX(void) {
  srand(15);
  global_seed = 1;
  randn_internal_controllerX(&global_seed, 1);
}
